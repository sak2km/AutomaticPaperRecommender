...
//						m_trainSet.add(docs.get(j));
	...
	public double test_corpus(_Corpus c)
	{
		...
		try {
			...
			for (int j = 0; j < docs.size(); j++) {
						m_testSet.add(docs.get(j));//Add all docs to test set
			}
			double accuracy = test();
				
			System.out.format("Predict with accuracy %.4f and F1 (%s)...\n",  accuracy, getF1String());
			m_trainSet.clear();
			m_testSet.clear();
