...
public class SVM extends BaseClassifier {
	...
	//Constructor with a given Model Added by Rizwan.
	...
	public void trainCorpus(_Corpus c) {
		m_libModel = libSVMTrain(c.getCollection(), 1+m_featureSize, m_type, m_C, 1);
	}
	 
	public void updateModel(double learning_rate, _Doc posDoc, _Doc negDoc){
		if(posDoc!=null){
			_SparseFeature[] del_x_i_j = Utils.diffVector(posDoc.getSparse(), negDoc.getSparse());
//			m_libModel.getWeights()[1423] = 0.661074869031906;
//			System.out.println(m_libModel.getWeights()[1423] );
			for(_SparseFeature fv: del_x_i_j)
			{
				m_libModel.getWeights()[1+fv.getIndex()]+= learning_rate* fv.getValue();
			}
//				node[fid++] = new FeatureNode(1 + fv.getIndex(), fv.getValue());//svm's feature index starts from 1
		}
		else{
			System.out.println("only subtracting as all docs are neg");
			_SparseFeature[] del_x_i_j = negDoc.getSparse();
			for(_SparseFeature fv: del_x_i_j)
			{
				m_libModel.getWeights()[1+fv.getIndex()]-= learning_rate* fv.getValue();
			}
	...
	{
		m_libModel = loadModelParametes(modelFile);
	}
	
	
	private static Model loadModelParametes(File modelFile) throws IOException{
		return Linear.loadModel(modelFile);
	}
//	public void testSVM(_Corpus c)
//	{
//		test();
//	}
